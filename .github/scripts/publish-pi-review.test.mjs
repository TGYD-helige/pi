...
test('prepares the review prompt outside the workflow', async () => {
  ...
  assert.match(workflow, /npm install --global --ignore-scripts @earendil-works\/pi-coding-agent@latest\b/);
  assert.match(workflow, /npm install --prefix "\$PI_CODING_AGENT_DIR\/review-runtime" --ignore-scripts pi-subagents@latest\b/);
  assert.match(workflow, /pi install npm:@ff-labs\/pi-fff@latest\b/);
  assert.match(workflow, /pi install npm:@amaster\.ai\/pi-telemetry@latest\b/);
