...
export async function preparePiReview({
  ...
  const reviewContext = [
    ...
    'all using the general-purpose agent. Its workflowScript must be exactly return await runs.all(<JSON array literal>);.',
    ...
    'Every task must set agentScope to "user" and cwd to the PR workspace, and contain only key, agent, agentScope, cwd, task, and outputSchema.',
    'Use the unique task keys standards, spec, and ponytail. Set async to false explicitly.',
