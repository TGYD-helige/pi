...
export async function preparePiReview({
  ...
  const reviewContext = [
    ...
    'for an independent over-engineering pass. Make exactly one synchronous subagent call with a tasks array containing exactly three tasks,',
    'all using the general-purpose agent.',
    ...
    'Every task may contain only agent, task, and outputSchema.',
    'Set agentScope to "user" and artifacts to false on the subagent call.',
    'Set cwd to the PR workspace on the subagent call. Set async to false explicitly.',
