...
export class ParallelProvider extends BaseProvider {
  ...
  async search(
    ...
    const request = async (
      ...
      if (message?.error)
        throw new Error(message.error.message ?? 'Parallel MCP returned an error.');
    ...
    try {
      await request('initialize', {
        ...
        capabilities: {},
        clientInfo: { name: 'pi', version },
      });
      await request('notifications/initialized', undefined, true);
      const listed = await request('tools/list');
      if (!listed?.tools?.some((tool: { name?: string }) => tool.name === 'web_search'))
        throw new Error('Parallel MCP web_search tool is unavailable.');
      ...
      const structured =
        ...
        called?.content
          ...
          .filter(Boolean)
          .map((text: string) => JSON.parse(text))[0];
      const results = Array.isArray(structured?.results)
        ? structured.results.slice(0, params.maxResults ?? 5)
        : [];
      return {
        provider: this.id,
        query: params.query,
        results: results.map(
          (item: { title?: string | null; url: string; excerpts?: string[] }) => ({
