...
Set the repository Actions variable or secret `PI_INTEGRATION_MODEL` (variable takes precedence) to select the text model on the existing integration gateway; unset or empty defaults to `deepseek-v4-flash`. Integration and Skill Eval use this selection with a 131072-token output limit, while embedding, image and browser vision models retain their separate settings. The gateway protocol stays unchanged. Workflow changes under `pull_request_target` take effect after reaching the base branch.
